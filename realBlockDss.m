function dss = realBlockDss(y_dct_img,y_sub_dct, sigma, C,repeat_height, repeat_width)
% img1 = y_sub_dct;
% img2 = y_sub_dct;
...
% img1Decomp = dct_decomp(y_dct_img);
img2Decomp = dct_decomp(y_sub_dct);
% img1Decomp = zeros(8*repeat_height, 8*repeat_width);
% img2Decomp = zeros(8*repeat_height, 8*repeat_width);

% img1Decomp = y_dct_img;
% img2Decomp = y_sub_dct;


% dss_out = zeros(8,8,255);
%    for i = 1:repeat_height
%         for j = 1:repeat_width
% %             y_compressed_img(((i-1)*8+1):(i*8), ((j-1)*8+1):(j*8)) = (reshape(y_sub_dct{i,j},8,8))';
%                 img2Decomp(((i-1)*8+1):(i*8), ((j-1)*8+1):(j*8)) = y_sub_dct{i,j};
%         end
%     end
...
%         for qpstep=1:255
%              img1Decomp(m:8:end, n:8:end) = round(img2Decomp(m:8:end, n:8:end)/qpstep)*qpstep;
            subbandSimilarity(m,n) = subband_similarity( ...
            img1Decomp(m:8:end, n:8:end), ...
            img2Decomp(m:8:end, n:8:end), ...
            m, n, C);
%            dss_out(m,n,qpstep)=subbandSimilarity(m,n);
%          end  
    end
%      dss_out(8,8,qpstep)=subbandSimilarity;
end

% Weight subbands similarity scores
 dss = 1/sum(sum(subbandSimilarity .* (w/sum(w(:)))));
