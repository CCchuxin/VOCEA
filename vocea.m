...
%      population2(i,:) = out;    
%   end 
%     functionvalue = MSE(population2, y_image, y_sub_image, y_sub_dct, repeat_height, repeat_width, outputName, 0, dss_table, w, rate_table);
...
%    functionvalue_gen = MSE(populationlumnew2, y_image, y_sub_image, y_sub_dct, repeat_height, repeat_width, outputName, 1, dss_table, w, rate_table);
%    hold on;
...
%      if gene==10
%          dlmwrite('DSSresult/kodim04_10.txt',functionvalue);
%      else if gene==30
%              dlmwrite('DSSresult/kodim04_30.txt',functionvalue);
%          else if gene ==50
%                  dlmwrite('DSSresult/kodim04_50.txt',functionvalue);
%              end
%          end
%      end
% figure;
...
%     truevalue = MSE(populationlum2(1,:), y_image, y_sub_image, y_sub_dct, repeat_height, repeat_width, outputName, 0, dss_table,w, rate_table);
    fprintf('已完成,耗时%4s秒\n',num2str(toc));          %程序最终耗时

    truevalue = MSE(populationlum2, y_image, y_sub_image, y_sub_dct, repeat_height, repeat_width, outputName, 0, dss_table,w, rate_table);
    estvalue = MSE(populationlum2, y_image, y_sub_image, y_sub_dct, repeat_height, repeat_width, outputName, 1, dss_table,w, rate_table);
    X = [0.2,0.6,1,1.4];%可行域
    Y = abs(X - truevalue(:,1));
    ...
    % bar(x,y);
    hold on
    bar(0.2,y1,0.05,'FaceColor',[0.75 0.75 0.75]);
    hold on
    bar(0.6,y2,0.05,'FaceColor',[0.75 0.75 0.75]);
    hold on
    bar(1,y3,0.05,'FaceColor',[0.75 0.75 0.75]);
    hold on
    bar(1.4,y4,0.05,'FaceColor',[0.75 0.75 0.75]);
    unCov_truevalue = load(unCov_truevalueName);
     ...
     hold on;
     plot(output(:,1),output(:,2),'^b');%双种群估算的解
%      hold on;
%      plot(jpeg(:,1),jpeg(:,2),'og');

     hold on;
     plot(unCov_truevalue(:,1),unCov_truevalue(:,2),'*b');%没有约束的单种群的解
     hold on;
