...
%     end
% end
repeat_height = size(img, 1)/8;
repeat_width = size(img, 2)/8;
repeat_height_mat = repmat(8, [1 repeat_height]);
repeat_width_mat = repmat(8, [1 repeat_width]);
y_sub_image = mat2cell(img, repeat_height_mat, repeat_width_mat);
decomp = zeros(nRows,nCols);
for i=1:1:(nRows/8)
    for j=1:1:(nCols/8)
        y_sub_image{i, j} = dcTransform(y_sub_image{i, j});
        y_sub_dct{i,j} = getDCT(y_sub_image{i,j});
        decomp(((i-1)*8+1):(i*8), ((j-1)*8+1):(j*8)) =  y_sub_dct{i,j};
    end
end
...
window = fspecial('gaussian', 3, 1.5); %´´½¨ÂË²¨Æ÷
