function [out bits_block] = huffman(fid, array, prev, DCcode, ACcode)
    ...
    bit_block = bit_block + DC_matrix(DCcode, nbits+1, 2);

    if nbits ~= 0
        bit_block = bit_block + nbits;
    end
    ...
    for k = 2:64 
        ...
        if temp == 0
            ...
            while r > 15 
                bit_block = bit_block + AC_matrix(ACcode, hex2dec('F0')+1, 2);
            ...
            bit_block = bit_block + AC_matrix(ACcode, i+1, 2);
            bit_block = bit_block + nbits;
    ...
    if r > 0
        ...
        bit_block = bit_block + AC_matrix(ACcode, 1, 2);
    end
    
    out = array(1);
    bits_block = bit_block;
end
