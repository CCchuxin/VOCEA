...
% [dss_table,w] = estBlockDss(y_sub_dct,1.55, [1000 300],repeat_height, repeat_width);
...
% %  basevalue_est = MSE(population, y_sub_image, y_sub_dct,repeat_height, repeat_width, outputName, 0, dss_table,w, rate_table);
%   basevalue_est = MSE(population, y_image, y_sub_image, y_sub_dct,repeat_height, repeat_width, outputName, 0, dss_table,w, rate_table);
% % %  est = MSE(population, y_image, y_sub_image, y_sub_dct,repeat_height, repeat_width, outputName, 1, dss_table,w, rate_table);
%   figure;
%  %   dlmwrite('resultDSS_JQF/kodim24.txt',basevalue_est);
%    plot(basevalue_est(:,1),basevalue_est(:,2),'og'); 
%   xlabel('Rate(bpp)');ylabel('1/DSS');
%  PF = [basevalue_est(population1,1),basevalue_est(1,2)];
...
%        CV = CalCV(cov);
%         if mean(CV<=0) > 0.5
%             Fitness = CalFitness(functionvalue,CV);
%         else
%             Fitness = CalFitness([CalSDE(functionvalue)',CV]);
%         end
    Fitness = CalFitness(functionvalue,CV); %计算适应度值
    [populationlum,Fitness] = EnvironmentalSelection(Fitness,newpopulation,functionvalue,population1);   
% %     functionvalue2 = MSE(populationlum_enviroment, y_image, y_sub_image, y_sub_dct, repeat_height, repeat_width, outputName, 1, dss_table, w, rate_table);
% %    [frontvalue, ~] = NDSort(functionvalue, inf);
% %   
% % % %    newnum = find(frontvalue<=1);
% % % %     front_index = find(frontvalue<=1);
% % % %     cv_index = find(cov<=0.1);
% % % %     index = intersect(front_index,cv_index);
% %      newnum=numel(frontvalue,frontvalue<=1);                              %前fnum个面的个体数   
% % % %     if(size(index,1) <50)
% %     if(newnum <50)
% %   %        populationlum(1:newnum,:)=populationlum_enviroment(frontvalue<=1,:);
% %          populationlum(1:newnum,:)=newpopulation(frontvalue<=1,:);
% %  %         populationlum(1:size(index,1),:)=newpopulation(index,:);
% %     else
% %    %      populationlum_temp(1:newnum,:)=populationlum_enviroment(frontvalue<=1,:);
% %          populationlum_temp(1:newnum,:)=newpopulation(frontvalue<=1,:);
% %  %        populationlum_temp(1:size(index,1),:)=newpopulation(index,:);
% % %         % convex hull
...
% % %                   functionvaluenew=MSE(populationlum(n,:), y_sub_image, y_sub_dct, repeat_height, repeat_width, outputName, 1, mse_table, rate_table);
% % %                   n = n+1;%5.6修改
% % %                   plot(functionvalue_gen(:,1),functionvalue_gen(:,2),'*g');
% % %                   hold on;
% % %                   plot(functionvaluenew(:,1),functionvaluenew(:,2),'or');
% %               end     
% %           end
% %       end
% %     
% % %     end
% %     
% %      populationlum = [populationlum;populationlum_enviroment];
    

    
%       functionvalue1=MSE(populationlum, y_sub_image, y_sub_dct, repeat_height, repeat_width, outputName, 1, dss_table,w, rate_table);
  functionvalue = MSE(populationlum, y_image, y_sub_image, y_sub_dct, repeat_height, repeat_width, outputName, 1, dss_table, w, rate_table); 
 

%       figure;
%       plot(functionvalue(:,1),functionvalue(:,2),'*r');
%      hold on;
%      plot(functionvalue1(:,1),functionvalue1(:,2),'ob');
    ...
    fprintf('已完成,耗时%4s秒\n',num2str(toc));          %程序最终耗时
%     dlmwrite('DSSresult/kodim24/time.txt',num2str(toc));
    truevalue = MSE(populationlum, y_image, y_sub_image, y_sub_dct, repeat_height, repeat_width, outputName, 0, dss_table,w, rate_table);
     dlmwrite('DSSresult/kodim01/truevalue.txt',truevalue);
     dlmwrite('DSSresult/kodim01/QT.txt',populationlum);

    estvalue = MSE(populationlum, y_image, y_sub_image, y_sub_dct, repeat_height, repeat_width, outputName, 1, dss_table,w, rate_table);
    X = [0.2,0.6,1,1.4];%可行域
    Y = abs(X - truevalue(:,1));
...
%     [functionvalue_gen,x] = sortrows(truevalue_new);
...
%     dlmwrite('DSSresult/kodim24/populationlum.txt',populationlum(indextrue,:));
%     dlmwrite('DSSresult/kodim24/truevalue.txt',truevalue_new);
    figure;
    y1 = 3;
    y2 = 1.5;
    y3 = 1.5;
    ...
    hold on
    bar(1.4,y4,0.1)
    unCov_truevalue = load('unCov_truevalue/kodim01_truevalue.txt');
